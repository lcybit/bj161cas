...
import com.jefflee.entity.schedule.*;
...
import com.jefflee.service.schedule.*;
import com.jefflee.util.*;
import com.jefflee.view.*;
...
public class ArrangementServiceImpl implements ArrangementService {
    ...
    public ScheduleView gnrScheduleView(Integer scheduleId) throws Exception {
        ...
        List<Arrangement> scheduleArrangementList = cache.getSemesterArrangementList();
        ...
        for (Plan plan : schedulePlanList) {
            ...
            if (tclassId != 0 && courseType != 4) {
                ...
                if (!tclassWeekViewMap.containsKey(tclassWeekViewId)) {
                    ...
                    titleView.setTclassName(grade.getShortName() + tclass.getTclassNo() + "班");
                    // 必要时添加
            ...
            if (teacherId != 0 && courseType != 3) {
                ...
                if (courseType != 1 && courseType != 4) {
                    if (!teacherWeekViewMap.containsKey(teacherWeekViewId)) {
                        ...
                        titleView.setGradeName(schedule.getGrade().getShortName());
        ...
        scheduleView.setTclassWeekViewMap(sortTclassWeekViewMap(tclassWeekViewMap));
        scheduleView.setTeacherWeekViewMap(sortTeacherWeekViewMap(teacherWeekViewMap));
        return scheduleView;
    }

    private Map<String, WeekView> sortTclassWeekViewMap(Map<String, WeekView> weekViewMap) {
        List<Map.Entry<String, WeekView>> entryList = new ArrayList<Map.Entry<String, WeekView>>(weekViewMap.entrySet());
        Collections.sort(entryList, new Comparator<Map.Entry<String, WeekView>>() {
            @Override
            public int compare(Map.Entry<String, WeekView> o1, Map.Entry<String, WeekView> o2) {
                Integer tclassId1 = Integer.parseInt(o1.getKey().split("-")[1]);
                Integer tclassId2 = Integer.parseInt(o2.getKey().split("-")[1]);
                Tclass tclass1 = tclassService.selectById(tclassId1);
                Tclass tclass2 = tclassService.selectById(tclassId2);
                Integer tclassNo1 = tclass1.getTclassNo();
                Integer tclassNo2 = tclass2.getTclassNo();
                return tclassNo1.compareTo(tclassNo2);
            }
        });
        Map<String, WeekView> map = new LinkedHashMap<String, WeekView>();
        for (Map.Entry<String, WeekView> entry : entryList) {
            map.put(entry.getKey(), entry.getValue());
        }
        return map;
    }

    private Map<String, WeekView> sortTeacherWeekViewMap(Map<String, WeekView> weekViewMap) {
        List<Map.Entry<String, WeekView>> entryList = new ArrayList<Map.Entry<String, WeekView>>(weekViewMap.entrySet());
        Collections.sort(entryList, new Comparator<Map.Entry<String, WeekView>>() {
            @Override
            public int compare(Map.Entry<String, WeekView> o1, Map.Entry<String, WeekView> o2) {
                String[] keys1 = o1.getKey().split("-");
                String[] keys2 = o2.getKey().split("-");
                Integer teacherId1 = Integer.parseInt(keys1[1]);
                Integer teacherId2 = Integer.parseInt(keys2[1]);
                Integer courseId1 = Integer.parseInt(keys1[3]);
                Integer courseId2 = Integer.parseInt(keys2[3]);
                Course course1 = courseService.selectById(courseId1);
                Course course2 = courseService.selectById(courseId2);
                Integer courseNo1 = course1.getCourseNo();
                Integer courseNo2 = course2.getCourseNo();
                int courseComparison = courseNo1.compareTo(courseNo2);
                return courseComparison == 0 ? teacherId1.compareTo(teacherId2) : courseComparison;
            }
        });
        Map<String, WeekView> map = new LinkedHashMap<String, WeekView>();
        for (Map.Entry<String, WeekView> entry : entryList) {
    ...
    private TitleView gnrTitleView(Schedule schedule) throws Exception {
        ...
        titleView.setStartDate(calcStartDate(startDate, startWeek));
        return titleView;
    }

    private String calcStartDate(String startDate, Integer startWeek) throws Exception {
    ...
    public void exportExcelView(HttpServletResponse response, Integer scheduleId) throws Exception {
        ...
        String startDate = grade.getStartDate();
        Integer startWeek = schedule.getStartWeek();
        String fileName = gradeName + calcStartDate(startDate, startWeek) + "课表.xlsx";
